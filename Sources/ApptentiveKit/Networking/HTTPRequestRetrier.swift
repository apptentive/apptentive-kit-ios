    ...
    func start<T>(_ builder: HTTPRequestBuilding, identifier: String) async throws -> T where T: Decodable & Sendable {
        ...
        let task = Task<Decodable & Sendable, Error> {
            while true {
                ...
                } catch HTTPClientError.serverError(let response, let data), HTTPClientError.retriableClientError(let response, let data) {
                    let responseString = data.flatMap { String(data: $0, encoding: .utf8) } ?? "<no text response>"
                    Logger.network.info("Retriable error sending API request with identifier ”\(identifier)”: \(response.statusCode) \(responseString).")
                } catch let error {
                    throw error
                }
