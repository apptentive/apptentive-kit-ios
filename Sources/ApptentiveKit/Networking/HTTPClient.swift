...
final class HTTPClient: Sendable {
    ...
    static func processResult(data: Data?, response: URLResponse?) throws -> HTTPResponse {
        ...
        case 429:
            throw HTTPClientError.retriableClientError(httpResponse, data)
        case 400...499:
    ...
    static func log(_ response: HTTPResponse) {
        ...
        default:
            ...
            #else
                if let stringValue = String(data: response.data, encoding: .utf8) {
                    Logger.network.debug("Body: <prefix> \(stringValue))")
                } else {
                    Logger.network.debug("Body: <not decodable as UTF-8>")
                }
            #endif
        }
    }
...
enum HTTPClientError: Error {
    ...
    case retriableClientError(HTTPURLResponse, Data?)
}
...
extension HTTPClientError: LocalizedError {
    var errorDescription: String? {
        ...
        case .retriableClientError(let response, let data):
            var message = "Retriable client error: \(response.statusCode)."
            if let data = data, let responseString = String(data: data, encoding: .utf8) {
                message = "\(message) Response object: \(responseString)"
            }
            return message
