...
protocol PayloadSending: Actor {
    ...
    func drain() async
    func resume() async
    func updateCredentials(_ credentials: PayloadStoredCredentials, for tag: String, authenticationContext: Payload.Context.AuthenticationContext) async throws
...
actor PayloadSender: PayloadSending {
    ...
    func updateCredentials(_ credentials: PayloadStoredCredentials, for tag: String, authenticationContext: Payload.Context.AuthenticationContext) async throws {
        if case .secure(let id) = credentials {
            self.conversationCredentials = .init(id: id, token: authenticationContext.token)
        }

        for (index, payload) in self.payloads.enumerated() {
            if payload.tag == tag {
                try self.payloads[index].updateCredentials(credentials, using: self.jsonEncoder, decoder: self.jsonDecoder, authenticationContext: authenticationContext)
            }
        }
    ...
    /// The credentials for the currently-active anonymous conversation, if any.
    ...
    private func sendPayloads() {
        ...
        let credentials = PayloadAPICredentials(appCredentials: appCredentials, payloadCredentials: firstPayload.credentials, conversationCredentials: self.conversationCredentials)
