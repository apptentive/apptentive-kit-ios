...
/// The portions of the host context that provide access to platform features.
@MainActor protocol GlobalContext {
    var fileManager: FileManaging { get }
    var isInForeground: Bool { get }
    var isProtectedDataAvailable: Bool { get }
    var delegate: HostContextDelegate? { get set }
...
/// Allows the HostContext to communicate changes.
@MainActor protocol HostContextDelegate: AnyObject {

    /// Notifies the receiver that access to protected data (from the encrypted filesystem) is now available.
    /// - Parameter context: The host context calling the method.
    func protectedDataDidBecomeAvailable(_ context: GlobalContext)
...
@MainActor final class HostContext: GlobalContext {
    ...
    /// The delegate to notify when aspects of the host context change.
    weak var delegate: HostContextDelegate?

    /// Initializes a new host context based on values captured from the operating system.
    init() {
