...
extension Apptentive {
    ...
    @objc public static var fontName: String? {
        didSet {
                ...
                let calloutFont = UIFont(name: fontName, size: 16.0)
            {
                ...
                UIFont.apptentiveDialogTitle = UIFontMetrics(forTextStyle: .headline).scaledFont(for: headlineFont)
                UIFont.apptentiveDialogText = UIFontMetrics(forTextStyle: .body).scaledFont(for: bodyFont)
                UIFont.apptentiveDialogMessage = UIFontMetrics(forTextStyle: .footnote).scaledFont(for: footnoteFont)
                UIFont.apptentiveDialogButton = UIFontMetrics(forTextStyle: .body).scaledFont(for: bodyFont)
            } else {
                ...
                UIFont.apptentiveDialogTitle = .preferredFont(forTextStyle: .headline)
                UIFont.apptentiveDialogText = .preferredFont(forTextStyle: .body)
                UIFont.apptentiveDialogMessage = .preferredFont(forTextStyle: .footnote)
                UIFont.apptentiveDialogButton = .preferredFont(forTextStyle: .body)
            }
        }
    }
    ...
    static func applyCustomerTheme(from bundle: Bundle = .main) {
        ...
        UIColor(named: "Apptentive/Dialog/Background", in: bundle, compatibleWith: nil).flatMap {
            UIColor.apptentiveDialogBackground = $0
            UIVisualEffectView.appearance(whenContainedInInstancesOf: [DialogView.self]).effect = .none
        }
        UIColor(named: "Apptentive/Dialog/TitleText", in: bundle, compatibleWith: nil).flatMap { UIColor.apptentiveDialogTitle = $0 }
        UIColor(named: "Apptentive/Dialog/MessageText", in: bundle, compatibleWith: nil).flatMap { UIColor.apptentiveDialogMessage = $0 }
        UIColor(named: "Apptentive/Dialog/LegacySeparator", in: bundle, compatibleWith: nil).flatMap { UIColor.apptentiveDialogSeparator = $0 }
        UIColor(named: "Apptentive/Dialog/ButtonLabel", in: bundle, compatibleWith: nil).flatMap { UIColor.apptentiveDialogButtonLabel = $0 }
        UIColor(named: "Apptentive/Dialog/ButtonBackground", in: bundle, compatibleWith: nil).flatMap { UIColor.apptentiveDialogButtonBackground = $0 }
        UIColor(named: "Apptentive/Dialog/LegacyButtonBackground", in: bundle, compatibleWith: nil).flatMap {
            DialogButton.appearance().backgroundColor = $0
        }
        ...
        UIImage(named: "Apptentive/Dialog/Header", in: bundle, compatibleWith: nil).flatMap { UIImage.apptentiveDialogHeader = $0 }
    }
    ...
    static func applyApptentiveTheme() {
        ...
        UIColor.apptentiveDialogTitle = dialogText
        UIColor.apptentiveDialogSeparator = dialogSeparator
        UIColor.apptentiveDialogButtonLabel = dialogButtonText
        UIColor.apptentiveDialogBackground = apptentiveGroupPrimaryColor
