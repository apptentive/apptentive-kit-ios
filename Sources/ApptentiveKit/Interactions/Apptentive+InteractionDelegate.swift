...
extension Apptentive {
    ...
    func requestReview() async throws -> Bool {
        ...
        return try await self.hostContext.requestReview()
    }
    ...
    func open(_ url: URL) async -> Bool {
        ...
        return await self.hostContext.open(url)
    }
    ...
    func addDraftAttachment(url: URL) async throws -> URL {
        ...
        // Preserve the file extension from the original URL
        let fileExtension = url.pathExtension
        let tempFilename = fileExtension.isEmpty ? UUID().uuidString : "\(UUID().uuidString).\(fileExtension)"
        let tempURL = URL(fileURLWithPath: tempFilename, relativeTo: FileManager.default.temporaryDirectory)

        // Use security-scoped resource access and file coordinator for files from UIDocumentPicker
        ...
        fileCoordinator.coordinate(readingItemAt: url, options: [], error: &coordinatorError) { sourceURL in
            do {
                try self.hostContext.fileManager.copyItem(at: sourceURL, to: tempURL)
            } catch {
                copyError = error
            }
        }
        ...
        return try await self.backend.addDraftAttachment(url: tempURL, thumbnailSize: thumbnailSize, thumbnailScale: thumbnailScale)
    }
    ...
    func recordEnjoyment(_ didLove: Bool, from interaction: Interaction) {
        ...
        Task {
            await self.backend.invalidateEngagementManifest(after: Date(timeIntervalSinceNow: 3600))
        }
    }
}
