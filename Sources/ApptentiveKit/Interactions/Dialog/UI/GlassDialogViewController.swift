...
class GlassDialogViewController: UIViewController, UIGestureRecognizerDelegate, DialogViewModelDelegate {
    ...
    var upperSpaceConstraint = NSLayoutConstraint()
    var sizeClassObservation: UITraitChangeRegistration?
    ...
    init(viewModel: DialogViewModel) {
        ...
        self.sizeClassObservation = registerForTraitChanges([UITraitVerticalSizeClass.self]) { (self: Self, previousTraitCollection: UITraitCollection) in
            self.handleVerticalSizeClassChange()
        }
    }
    
    @MainActor
    deinit {
        self.sizeClassObservation.flatMap { unregisterForTraitChanges($0) }
    }
    ...
    override func viewDidLoad() {
        ...
        self.overallHeightConstraint = self.dialogView.heightAnchor.constraint(lessThanOrEqualTo: self.view.heightAnchor, constant: -128)
        self.upperSpaceConstraint = self.dialogView.topAnchor.constraint(greaterThanOrEqualTo: self.view.topAnchor, constant: 47)
        ...
        let verticalCenterConstraint = self.dialogView.centerYAnchor.constraint(equalTo: self.view.centerYAnchor)
        ...
        NSLayoutConstraint.activate([
            ...
            self.view.bottomAnchor.constraint(greaterThanOrEqualTo: self.dialogView.bottomAnchor, constant: 37),
            self.dialogView.widthAnchor.constraint(equalToConstant: 320),
            self.overallHeightConstraint,
            preferredHeightConstraint,
            self.upperSpaceConstraint,
        ])
    ...
    private func handleVerticalSizeClassChange() {
        if self.traitCollection.verticalSizeClass == .compact {
            self.overallHeightConstraint.constant = -21
            self.upperSpaceConstraint.constant = 8
        } else {
            self.overallHeightConstraint.constant = -128
            self.upperSpaceConstraint.constant = 47
        }
    }
