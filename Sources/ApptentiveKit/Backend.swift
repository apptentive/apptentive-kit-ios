...
protocol BackendProtocol: Actor {
    ...
    func setShouldUseSecureTokenStorage(_: Bool)

    func register(appCredentials: Apptentive.AppCredentials, region: Apptentive.Region, environment: Apptentive.Environment) async throws -> Backend.ConnectionType
    ...
    func setInteractionRateLimitPerSession(_ count: Int)
    func excludeEventsFromRateLimit(_ events: [Event])

    func invalidateEngagementManifest(after invalidationDate: Date?)
...
actor Backend: PayloadAuthenticationDelegate, BackendProtocol {
    ...
    var payloadContext: Payload.Context {
        ...
        case (.none, .none):
            return .init(tag: "loggedOut", credentials: .placeholder, sessionID: nil, encoder: self.jsonEncoder, authenticationContext: nil)

        case (.anonymous(let credentials), .some(let path)):
            return .init(tag: path, credentials: .secure(id: credentials.id), sessionID: self.sessionID, encoder: self.jsonEncoder, authenticationContext: nil)

        case (.loggedIn(let credentials, _, let encryptionKey), .some(let path)):
            return .init(tag: path, credentials: .embedded(id: credentials.id), sessionID: self.sessionID, encoder: self.jsonEncoder, authenticationContext: .init(encryptionKey: encryptionKey, token: credentials.token))

        default:
            return .init(tag: "placeholder", credentials: .placeholder, sessionID: self.sessionID, encoder: self.jsonEncoder, authenticationContext: nil)
        }
    }
    ...
    var interactionRateLimitPerSession = 1
    ...
    ///   - tokenStore: An object that can securely store and retrieve authentication tokens.
    init(
        ...
        Task {
            ...
            await self.excludeEventsFromRateLimit([.showMessageCenter, .showMessageCenterFallback])
        }
    ...
    func setShouldUseSecureTokenStorage(_ shouldUseSecureTokenStorage: Bool) {
        self.shouldUseSecureTokenStorage = shouldUseSecureTokenStorage
    }
    ...
    ///   - environment: The environment to use when building the API base URL.
    /// - Returns: A value that indicates whether the connection is new or cached.
    /// - Throws: An error if registration fails.
    func register(appCredentials: Apptentive.AppCredentials, region: Apptentive.Region, environment: Apptentive.Environment) async throws -> ConnectionType {
        let baseURL = try environment.resolve(appCredentials: appCredentials, region: region)
        let client = HTTPClient(requestor: self.requestor, baseURL: baseURL, userAgent: ApptentiveAPI.userAgent(sdkVersion: self.dataProvider.sdkVersion), languageCode: self.dataProvider.preferredLocalization)
    ...
    func setInteractionRateLimitPerSession(_ limit: Int) {
        self.interactionRateLimitPerSession = limit
    }

    func excludeEventsFromRateLimit(_ events: [Event]) {
        for event in events {
            self.rateLimitExemptCodePoints.insert(event.codePointName)
        }
    }
    ...
    @discardableResult func engage(event: Event) async throws -> Bool {
        ...
        if self.status?.metricsEnabled == true || event.vendor == "com.apptentive" || self.dataProvider.isDebugBuild {
            do {
            ...
            Logger.default.info("Metrics not enabled: Skip sending of app-engaged event payload.")
        }
            ...
            if let status = self.status {
                Task {
                    await self.messageManager.setPollingIntervals(
                        foreground: status.messageCenter.foregroundPollingInterval,
                        background: status.messageCenter.backgroundPollingInterval)
    ...
    private func handleTransition(_ transition: (from: BackendState.Summary, to: BackendState.Summary)) async {
        ...
        do {
            ...
            case (from: .posting, to: .anonymous(let payloadCredentials, let authenticationContext)):
            ...
            case (from: .anonymous, to: .loggedIn(let payloadCredentials, let authenticationContext)):
                ...
                try await self.payloadSender.updateCredentials(payloadCredentials, for: activeRecord.path, authenticationContext: authenticationContext)
            ...
            case (from: .loggedIn, to: .loggedIn(let payloadCredentials, let authenticationContext)):  // Conversation credentials were updated.
                guard let activeRecord = self.state.roster.active else {
                    throw ApptentiveError.internalInconsistency
                }
    ...
    private func maybeThrottleInteraction(_ interaction: Interaction, for event: Event) throws {
        ...
        let interactionIsRateLimited = interactionSessionCount >= self.interactionRateLimitPerSession
        ...
        if interactionIsRateLimited && !eventIsExempt && !interaction.isExemptFromRateLimit {
            // Temporarily disable client-side interaction throttling.
            // throw ApptentiveError.interactionExceededRateLimit(count: interactionSessionCount, limit: self.interactionRateLimitPerSession)
        }
    ...
    internal func syncConversationWithAPI() async {
        ...
        if let applicationID = self.state.roster.applicationID {
            ...
            Logger.default.info("Skipping status check due to missing application ID.")
        }
    ...
    private func getInteractionsIfNeeded(with credentials: AnonymousAPICredentials) {
        ...
        let needsFetch = self.targeter.engagementManifest.isPlaceholder || cacheExpired || cacheInvalidated

        // Check that the engagement manifest in memory (if any) is expired.
        Task {
            if await !self.requestRetrier.requestIsUnderway(for: "get interactions") && needsFetch {
                Logger.default.info("Requesting new engagement manifest via Apptentive API/URL cache (current one is absent or stale).")

                do {
                    let engagementManifest: EngagementManifest = try await self.requestRetrier.start(ApptentiveAPI.getInteractions(with: credentials, shouldIgnoreCache: cacheInvalidated || cacheExpired), identifier: "get interactions")

                    self.targeter.engagementManifest = engagementManifest
                    await self.delegate?.prefetchResources(at: engagementManifest.prefetch ?? [])

                    if self.state.roster.applicationID == nil {
                        self.state.roster.applicationID = engagementManifest.applicationID
                        try self.saveRoster(self.state.roster)
                    } else if self.state.roster.applicationID != engagementManifest.applicationID {
                        apptentiveCriticalError("Manifest app ID does not match conversation roster app ID!")
                        self.state.fatalError = true
                    }
    ...
    private func getStatusIfNeeded(with credentials: StatusCredentials) {
        ...
        Task {
            if await !self.requestRetrier.requestIsUnderway(for: "get status") && (self.status == nil || statusExpired) {
                Logger.default.info("Requesting new application status via Apptentive API/URL cache (current one is absent or stale).")

                do {
                    let status: Status = try await self.requestRetrier.start(ApptentiveAPI.getStatus(with: credentials), identifier: "get status")
                    self.status = status
                    ...
                    Logger.network.debug("App status received from \(source). New expiry is \(status.expiry as? NSObject)")
                } catch let error {
                    Logger.network.error("Failed to download application status: \(error).")
                }
            }
        }
    }
