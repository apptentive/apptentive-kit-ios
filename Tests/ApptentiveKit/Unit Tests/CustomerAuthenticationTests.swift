...
actor CustomerAuthenticationTests {
    ...
    func createBackend(with roster: ConversationRoster) async {
        ...
        self.backend = Backend(
            ...
            payloadSender: self.payloadSender, dataProvider: self.dataProvider, fileManager: FileManager(), tokenStore: MockTokenStore())
    ...
    @Test func testLoginFromAnonymous() async throws {
        ...
        try await withTimeout(seconds: 2) {
            while await self.backend.conversation?.codePoints["com.apptentive#app#login"]?.totalCount ?? 0 == 0 {
    ...
    //            do {
    //                try self.backend.protectedDataDidBecomeAvailable(hostContext: self.backendDelegate.hostContext)
    //
    ...
    //                    appCredentials: Apptentive.AppCredentials(key: "abc", signature: "123"), hostContext: self.backendDelegate.hostContext
    //                ) { _ in
