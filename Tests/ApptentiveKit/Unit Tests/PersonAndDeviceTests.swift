...
struct PersonAndDeviceTests {
    var hostContext: MockHostContext!
    var apptentive: Apptentive!
    var dispatchQueue: DispatchQueue!

    init() throws {
        try MockHostContext.cleanContainerURL()

        self.dispatchQueue = DispatchQueue(label: "Test Queue")

        self.hostContext = MockHostContext()
        self.hostContext?.isProtectedDataAvailable = false
        let containerDirectory = "com.apptentive.feedback.\(UUID().uuidString)"

        self.apptentive = Apptentive(containerDirectory: containerDirectory, hostContext: self.hostContext)

        Apptentive.alreadyInitialized = false

        // Clean up data directory before running tests
        let containerURL = MockHostContext.applicationSupportURL.appendingPathComponent(containerDirectory)
    ...
    //        self.hostContext.isProtectedDataAvailable = true
    //
    //        self.apptentive.personName = "Testy McTestface"
    //        self.apptentive.personEmailAddress = "test@example.com"
    ...
    //                self.apptentive = Apptentive(baseURL: URL(string: "https://localhost"), containerDirectory: containerDirectory, backendQueue: self.dispatchQueue, hostContext: self.hostContext)
    //
