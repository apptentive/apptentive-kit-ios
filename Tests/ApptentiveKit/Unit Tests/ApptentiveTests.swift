...
struct ApptentiveTests {
    ...
    let hostContext = MockHostContext()
    let dataProvider = MockDataProvider()

    init() {
        let requestor = SpyRequestor(responseData: Data())
        Apptentive.alreadyInitialized = false
        self.apptentive = Apptentive(backend: backend, requestor: requestor, dataProvider: dataProvider, hostContext: hostContext)
    }
        ...
        await withCheckedContinuation { continuation in
            let result = self.apptentive.didReceiveRemoteNotification(
                ...
                fetchCompletionHandler: { result in
                    ...
                    Task {
                        #expect(await self.hostContext.localNotificationTitle == "This Nifty App")
                        #expect(await self.hostContext.localNotificationBody == "Foo")
                        #expect(await self.hostContext.localNotificationUserInfo?["apptentive"] == ["alert": "Foo"])
                        // Can't test due to sendability of UNNotificationSound
                        // #expect(await self.hostContext.localNotificationSound != nil)
    ...
    @Test func testAddDraftAttachmentURL() async throws {
        guard let hostContext = self.apptentive.hostContext as? MockHostContext, let fileManager = hostContext.fileManager as? MockFileManager else {
            throw TestError(reason: "Expected MockHostContext")
        }
    ...
    @Test func testProtectedDataAvailable() async throws {
        ...
        self.apptentive.protectedDataWillBecomeUnavailable(self.apptentive.hostContext)
        ...
        self.apptentive.protectedDataDidBecomeAvailable(self.apptentive.hostContext)
    ...
    @Test func testApplicationWillTerminate() {
        self.apptentive.applicationWillTerminate(self.apptentive.hostContext)
    }

    @Test func testEnvironmentResolution() throws {
        let appCredentials = Apptentive.AppCredentials(key: "abc", signature: "123")
        let region = Apptentive.Region.us

        let resolvedURL = try Apptentive.Environment.production.resolve(appCredentials: appCredentials, region: region)

        #expect(resolvedURL.host == "abc.api.digital.us.alchemer.com")
        #expect(resolvedURL.scheme == "https")
    }
}
...
actor SpyBackend: BackendProtocol {
    ...
    var payloadContext: Payload.Context = Payload.Context(tag: "tag", credentials: .embedded(id: "abc123"), encoder: JSONEncoder(), authenticationContext: .init(encryptionKey: Data(), token: "def456"))
    ...
    func register(appCredentials: ApptentiveKit.Apptentive.AppCredentials, region: ApptentiveKit.Apptentive.Region, environment: Apptentive.Environment) async throws -> ApptentiveKit.Backend.ConnectionType {
    ...
    func invalidateEngagementManifest(after: Date?) {
    }
    ...
    func excludeEventsFromRateLimit(_ events: [ApptentiveKit.Event]) {
    }
}
