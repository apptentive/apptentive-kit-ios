...
class LoaderTests: @unchecked Sendable {
    ...
    @Test func testALoading() throws {
        ...
        let roster = try CurrentLoader.loadLatestVersion(context: context) { loader in
            let roster = try loader.loadRoster(with: tokenStore)
    ...
    @Test func testBRosterWithKeychain() throws {
        let containerURL = try self.copyContainerToTemp(for: "B Keychain Data")
        defer {
            try? fileManager.removeItem(at: containerURL)
        }
        ...
        guard case .anonymous(let credentials) = activeRecord.state else {
            throw TestError(reason: "Migrated conversation is pending.")
        }
        ...
        #expect(credentials.id == "6830e00b3c9ec62b4a43b4f7")
    }
}
