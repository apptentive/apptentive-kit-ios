...
class SpyInteractionDelegate: InteractionDelegate {
    ...
    func loadAttachmentDataFromDisk() throws -> [Data] {
        let fileURL = URL(fileURLWithPath: "/tmp").appendingPathComponent(MockHostContext.containerName)
        let fileList = try self.hostContext.fileManager.contentsOfDirectory(at: fileURL)
    ...
    func saveAttachmentToDisk(fileName: String, index: Int, mediaType: String, data: Data) {
        ...
        do {
            let fileURL = URL(fileURLWithPath: "/tmp").appendingPathComponent(MockHostContext.containerName).appendingPathComponent(uniqueID).appendingPathExtension(fileExtension)
    ...
    func deleteAttachmentFromDisk(fileName: String, index: Int, mediaType: String) {
        ...
        do {
            let fileURL = URL(fileURLWithPath: "/tmp").appendingPathComponent(MockHostContext.containerName).appendingPathComponent(uniqueID).appendingPathExtension(fileExtension)
            try self.hostContext.fileManager.removeItem(at: fileURL)
        } catch {
