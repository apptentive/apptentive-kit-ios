...
struct HTTPRequestRetrierTests {
    ...
    @Test func testRetryOnServerError() async throws {
        let conversation = Conversation(dataProvider: MockDataProvider())
        let builder = ApptentiveAPI.createConversation(conversation, with: self.pendingCredentials, token: nil)
        let errorResponse = HTTPURLResponse(url: URL(string: "https://www.example.com")!, statusCode: 503, httpVersion: "1.1", headerFields: [:])!

        await self.requestor.setError(.serverError(errorResponse, nil))
        await self.requestor.setExtraCompletion({ requestor in
            Task {
                if let _ = await requestor.error {
                    await requestor.setError(nil)
    ...
    @Test func testRetryOnRetriableClientError() async throws {
        let conversation = Conversation(dataProvider: MockDataProvider())
        let builder = ApptentiveAPI.createConversation(conversation, with: self.pendingCredentials, token: nil)
        let errorResponse = HTTPURLResponse(url: URL(string: "https://www.example.com")!, statusCode: 503, httpVersion: "1.1", headerFields: [:])!

        await self.requestor.setError(.retriableClientError(errorResponse, nil))
        await self.requestor.setExtraCompletion({ requestor in
            Task {
                if let _ = await requestor.error {
                    await requestor.setError(nil)
