...
class MessagePayloadTests: XCTestCase {
    ...
    func testEncoding() throws {
        ...
        try checkPayloadEquivalence(between: self.testPayload.jsonObject, and: expectedEncodedContent, comparisons: ["automated", "hidden", "body"])
    }
    ...
    func testEncodingWithAttachments() throws {
        ...
        let actualDecodedContent: Payload.JSONObject = try jsonDecoder.decode(Payload.JSONObject.self, from: content0Data)
        ...
        try checkPayloadEquivalence(between: actualDecodedContent, and: expectedEncodedContent, comparisons: ["automated", "hidden", "body"])
