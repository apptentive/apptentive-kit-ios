...
struct ApptentiveAPITests {
    ...
    init() throws {
        self.payloadContext = Payload.Context(
            tag: ".", credentials: .header(id: anonymousCredentials.conversationCredentials.id, token: anonymousCredentials.conversationCredentials.token), sessionID: "abc123", encoder: JSONEncoder.apptentive, authenticationContext: nil)
    }
    ...
    @Test func testBuildMultipartRequest() throws {
        ...
        let endpoint = ApptentiveAPI(credentials: self.anonymousCredentials, path: path, method: method, bodyParts: [part1, part2, part3], shouldIgnoreCache: false)
    ...
    @Test func testParseExpiry() throws {
        let response1 = HTTPURLResponse(url: URL(string: "https://api.apptentive.com/foo")!, statusCode: 200, httpVersion: "1.1", headerFields: ["Cache-Control": "max-age = 86400", "Date": "Sat, 29 Apr 1982 01:23:45 GMT"])!

        guard let (expiry1, downloadTime1) = ApptentiveAPI.parseExpiry(response1) else {
            throw TestError(reason: "Unable to parse valid expiry")
        }

        #expect((expiry1.timeIntervalSince1970 == 388_977_825) ± 1.0)
        #expect((downloadTime1.timeIntervalSince1970 == 388_891_425) ± 1.0)
        ...
        guard let (expiry3, downloadTime3) = ApptentiveAPI.parseExpiry(response3) else {
            throw TestError(reason: "Unable to parse valid expiry (with weird case)")
        }

        #expect((expiry3.timeIntervalSince1970 == 388892075.0) ± 1.0)
        #expect((downloadTime3.timeIntervalSince1970 == 388_891_425) ± 1.0)
    }
    ...
    @Test func testGetInteractions() async throws {
        ...
        let _ = try await client.request(ApptentiveAPI.getInteractions(with: self.anonymousCredentials, shouldIgnoreCache: true))
        let request = await requestor.request
    ...
    @Test func testGetStatus() async throws {
        let baseURL = URL(string: "http://example.com")!
        let requestor = SpyRequestor(responseData: try JSONEncoder.apptentive.encode(ConversationResponse(token: "abc", id: "123", deviceID: "456", personID: "789", encryptionKey: nil)))
        let client = HTTPClient(requestor: requestor, baseURL: baseURL, userAgent: ApptentiveAPI.userAgent(sdkVersion: "1.2.3"), languageCode: "de")
        ...
        let _ = try await client.request(ApptentiveAPI.getStatus(with: .init(appCredentials: anonymousCredentials.appCredentials, applicationID: "def")))
        let request = await requestor.request
