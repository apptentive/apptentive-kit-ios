...
class PayloadSenderTests: PayloadAuthenticationDelegate, @unchecked Sendable {
    ...
    init() async throws {
        ...
        self.uncredentialedPayloadContext = Payload.Context(tag: ".", credentials: .placeholder, sessionID: "abc123", encoder: self.jsonEncoder, authenticationContext: nil)
    }
    ...
    @Test func testNormalOperation() async throws {
        ...
        try await payloadSender.updateCredentials(.header(id: self.anonymousCredentials.conversationCredentials.id, token: self.anonymousCredentials.conversationCredentials.token), for: ".", authenticationContext: .init(encryptionKey: nil, token: "abc"))

        // Send one straggler payload while the requests are running.
        let credentialedPayloadContext = Payload.Context(
            tag: ".", credentials: .header(id: self.anonymousCredentials.conversationCredentials.id, token: self.anonymousCredentials.conversationCredentials.token), encoder: self.jsonEncoder, authenticationContext: nil)
    ...
    @Test func testHTTPClientError() async throws {
        ...
        // Send one straggler payload while the requests are running.
        let credentialedPayloadContext = Payload.Context(
            tag: ".", credentials: .header(id: self.anonymousCredentials.conversationCredentials.id, token: self.anonymousCredentials.conversationCredentials.token), encoder: self.jsonEncoder, authenticationContext: .init(encryptionKey: nil, token: "abc")
        )
        let straggler = try Payload(wrapping: "test4", with: credentialedPayloadContext)
    ...
    @Test func testAddCredentials() async throws {
        let altPayloadContext = Payload.Context(tag: "alt", credentials: .placeholder, encoder: self.jsonEncoder, authenticationContext: nil)
